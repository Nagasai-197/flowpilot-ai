    ...
    try {
      ...
      try {
        const { data, error } = await supabase
          .from("profiles")
          .select(
            "working_hours_start, working_hours_end, preferred_deep_work_duration, break_duration, timezone",
          )
          .eq("id", userId)
          .single();
        ...
        logger.warn(
          `Failed to fetch preferred durations from profiles table: ${err.message}. Falling back.`,
        );
        const { data, error } = await supabase
          .from("profiles")
          .select("working_hours_start, working_hours_end, timezone")
      ...
      const deepWorkDuration =
        preferredDeepWorkDuration ?? (profile as any)?.preferred_deep_work_duration ?? 90;
      ...
      if (currentTime && dateStr) {
        ...
        if (isToday) {
          ...
          if (currentMs >= workEndMs) {
            ...
            logger.info(
              `Today planner execution called past working hours end ${stdEnd} for user ${userId}. Returning empty schedule.`,
            );
            ...
            return {
              schedule: [],
              recommendations: [
                "Your configured working hours for today are already over. Great job today, get some rest!",
              ],
            };
          }
          ...
          if (currentMs >= workStartMs) {
            ...
            logger.info(
              `Today planner execution inside work window. Skipping past slots. Effective start hour shifted from ${stdStart} to ${roundedTimeStr}`,
            );
      ...
      try {
        ...
        rawPlan = AIResponseValidator.fallbackGracefully(
          userId,
          targetDateStr,
          effectiveStart,
          effectiveEnd,
      ...
      let validatedPlan;
      try {
        validatedPlan = AIResponseValidator.validate(
          rawPlan,
          effectiveStart,
          effectiveEnd,
          targetDateStr,
        ...
        validatedPlan = AIResponseValidator.fallbackGracefully(
          userId,
          targetDateStr,
          effectiveStart,
          effectiveEnd,
