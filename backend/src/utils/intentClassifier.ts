...
*Next Best Action: Type **'Start my day'** to view your flagship briefing dashboard!*`,
    ...
    if (intent === 'life_score') {
      ...
      // Habit consistency
      const consistencyRatio = context.habits.length > 0 
      ...
      const todayTasks = context.tasks.filter(t => t.due_date && t.due_date.split('T')[0] === todayStr);
      ...
      const todayHabitRate = context.habits.length > 0 ? (todayLogs.length / context.habits.length) : 1.0;

      const todaySuccessScore = Math.min(100, Math.round(
        (0.4 * todayTaskRate + 0.4 * todayHabitRate + 0.2 * plannerAdherence) * 100
      ));

      // Category Sub-scores
      const workScore = Math.min(100, Math.round(taskRatio * 100));
      const healthScore = Math.min(100, Math.round(consistencyRatio * 100));
      const consistencyScore = context.habits.length > 0 ? Math.min(100, Math.max(...context.habits.map(h => h.streak)) * 8) : 80;

      let responseText = `Your unified **AI Life Score is ${lifeScore}/100** and today's accomplishments currently gauge at a **Success Score of ${todaySuccessScore}/100**!
...
*Judges Note: This represents your unified operating index, weighted for task execution, habit Streaks, and planned deep work blocks, penalized by outstanding overdue work.*`;
    ...
    if (intent === 'daily_briefing') {
      const highPriorityTasks = context.tasks.filter(t => t.status !== 'done' && t.priority === 'high');
      const focusBlocks = context.schedule.filter(b => b.type === 'focus');
      ...
      let responseText = `🌅 **Good morning! Welcome to your FlowPilot AI Daily Standup & Briefing**
...
*Let's conquer your day, pilot!*`;
    ...
    if (intent === 'productivity_analysis') {
      if (text.includes('score') || text.includes('how is') || text.includes('performance')) {
        ...
        if (score >= 80) {
          responseText += '🏆 Outstanding momentum! You are currently operating in your peak focus zone. Keep it up!';
        } else if (score >= 50) {
          responseText += '⚡ Steady focus. Log one more habit checks or tasks completion today to break into your peak performance zone.';
        } else {
          responseText += '🌿 A gentle reminder: Focus has its seasons. Try scheduling a small routine habit or a 15-minute inbox sprint to reset your day.';
        }
    ...
    if (intent === 'habit_coaching') {
      if (text.includes('streak') || text.includes('habits') || text.includes('meditation')) {
        ...
        const activeStreaks = context.habits.map((h) => `- **${h.name}**: ${h.streak}-day streak (30d success rate: ${h.pct}%)`);
